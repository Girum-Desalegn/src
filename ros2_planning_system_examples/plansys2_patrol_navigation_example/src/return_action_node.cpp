...
public:
  MoveAction()
  : plansys2::ActionExecutorClient("return", 500ms)
  {
    ...
    subscription_min_pose_ = create_subscription<geometry_msgs::msg::Pose>(
      "minimum_marker_pose",
      10,
      std::bind(&MoveAction::minimum_pose_callback, this, std::placeholders::_1));
    
  }

  void minimum_pose_callback(const geometry_msgs::msg::Pose::SharedPtr msg)
  {
    // Update the waypoint based on the subscribed custom topic
    RCLCPP_INFO(
      get_logger(),
      "Updating waypoint to new position: x=%.2f, y=%.2f",
      msg->position.x, msg->position.y);

    waypoints_["wp1"].pose.position.x = msg->position.x;
    waypoints_["wp1"].pose.position.y = msg->position.y;
  }
...
private:
  ...
  rclcpp::Subscription<geometry_msgs::msg::Pose>::SharedPtr subscription_min_pose_;
